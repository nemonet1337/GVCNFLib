...
public class GVCGunReloadtestGuiInventory extends GuiContainer
{
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
    	 ...
    	 if (button.id == 29)
         {
			...
			try {
				//File newfile = new File(mod_GVCLib.proxy.ProxyFile(),"mods" + File.separatorChar + "reloadmotion" + time + ".txt");
		//		String nname = gun.getUnlocalizedName();
		//		nname = "reload_" + nname.replace("item.", "") + ".txt";
				String nname = gun.reload_motion_name.substring(gun.reload_motion_name.lastIndexOf("/"));
				nname = gun.reload_motion_name.substring(gun.reload_motion_name.lastIndexOf("/") +1);
				File newfile = new File(mod_GVCLib.proxy.ProxyFile(),"mods" + File.separatorChar +  nname);
