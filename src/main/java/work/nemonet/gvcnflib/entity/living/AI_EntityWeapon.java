...
public abstract class AI_EntityWeapon
{
    		...
    		String texf = gun.bulletf_tex;
        	{
        		...
        		if(entity.counter1 && entity.gun_count1 > gun.mobcycle && entity.getRemain_L() > 0){
        			...
        			AI_EntityWeapon.Attacktask(entity, entity, gun.mobbullettype, gun.mobrange, 
    						...
    						gun.powor/2, gun.speed * 0.9F, gun.bure, gun.exlevel, ex, gun.shotgun_pellet, gun.gra, gun.bullet_time_max, 0);
    				entity.gun_count1 = 0;
    				if(entity.firetrue){
	...
	public static void Biped_Sword(EntityGVCLivingBase entity, ItemStack itemstack){
		...
		if(task){//1if(task && !entity.getSneak())
	...
	public static void getTargetEntity(EntityGVCLivingBase entity, double range, double maxy, double miny){
		...
		/*if(entity.biped) {
			if (entity.isPotionActive(MobEffects.BLINDNESS) || (entity.isPotionActive(MobEffects.SLOWNESS))) {
		...
		{
			//if(entity.targetentity == null && entity.world.getWorldTime() %5 == 0)//if(!entity.world.isRemote && entity.targetentity == null && entity.ontick %5 == 0)
			{
		...
		//					break;
				...
				if(target != null) {
					...
					entity.world.spawnParticle(EnumParticleTypes.NOTE, entity.posX,  entity.posY + entity.height + 1, entity.posZ, 0, 0, 0);
				}
			}
		}
	...
	public static boolean getRange(EntityGVCLivingBase entity, double range, double maxy, double miny){
		...
		//				boolean flag = entity.getEntitySenses().canSee(entity1);
			            ...
			            boolean pitch = (rotep_offset <= entity.rotationp_min && rotep_offset >= entity.rotationp_max);
			            boolean hightp = (entity1.posY < dyp && entity1.posY > dym);
			            
			           if( entity.mob_attackrange_hight) {
			        	   /*if(entity1.posY > dym) {
	...
	/**　上のgetRangeからvehicleとmob_attackrange_hightを追加しただけ*/
	public static boolean getRange_new(EntityGVCLivingBase entity, EntityGVCLivingBase vehicle, double range, double maxy, double miny){
		boolean task = false;
		double k = entity.posX;
		double l = entity.posY;
		double i = entity.posZ;
		double han = range;
		double y_range = range;
		if(Math.abs(range) > Math.abs(miny) && Math.abs(range) > Math.abs(maxy) ) {
			y_range = Math.abs(range);
		}
		else if(Math.abs(miny) > Math.abs(range) && Math.abs(miny) > Math.abs(maxy) ) {
			y_range = Math.abs(miny);
		}
		else if(Math.abs(maxy) > Math.abs(range) && Math.abs(maxy) > Math.abs(miny) ) {
			y_range = Math.abs(maxy);
		}
		AxisAlignedBB axisalignedbb = (new AxisAlignedBB((double) (k - han), (double) (l - y_range),(double) (i - han), 
				(double) (k + han), (double) (l + y_range), (double) (i + han)))
						.grow(1);
		List<Entity> llist = entity.world.getEntitiesWithinAABBExcludingEntity(entity,axisalignedbb);
				//entity.getEntityBoundingBox().expand(entity.motionX, entity.motionY, entity.motionZ).grow(range));
		if (llist != null) {
		...
		//				boolean flag = entity.getEntitySenses().canSee(entity1);
			           ...
			           if(vehicle.mob_attackrange_hight) {
			        	   //System.out.println("debug0");
			        	   if(pitch && hightp) {
			        		   //System.out.println("debug00001");
								if(entity1.isRiding() && entity1.getRidingEntity() != null && entity1.getRidingEntity() instanceof EntityLivingBase) {
									EntityLivingBase en = (EntityLivingBase) entity1.getRidingEntity();
									if(en.getHealth()> 0.0F ) {
										task = true;
	...
	public static void AIWeapon_new1(EntityGVCLivingBase entity, EntityGVCLivingBase onride, int id, double range,
    	...
    	if(id < 10){
			for (int ka = 0; ka < kazu; ++ka) {
				...
				flash.setTex(texf);
				flash.setLocationAndAngles(px + xx11, py + h, pz + zz11, entity.rotationYaw,
						entity.rotationPitch);
	...
	public static void AIWeapon_new2(EntityGVCLivingBase entity, EntityGVCLivingBase onride, int id, double range,
    	...
    	int ra = entity.world.rand.nextInt(10) + 1;
    	float val = ra * 0.02F;
    	if(sound != null)entity.playSound(sound, 3.0F, 0.9F + val);
		entity.firetrue = true;
    }
}