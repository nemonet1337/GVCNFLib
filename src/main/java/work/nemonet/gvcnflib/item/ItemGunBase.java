...
import objmodel.IModelCustom;
	...
	public class ItemGunBase extends ItemBow {
	  ...
	  //rotation
	    ...
	    public int mat_rk_leftarm = 3;
	    public String[] mat_r_leftarm = new String[]{"leftarm", "0", "15", 
	    		"0", "0", "0", //pos
	    		"0", "0", "-1", //off
	    		"0", "0", "0", //roteoff
	    		"0", "-1", "0", //move
	...
	//	public float noz;
		...
		//03/01
		  ...
		  //24/01/07
			...
			public float missile_targeting_size = 1.0F;
...
//				if (pReset) resetBolt(pItemstack);
		...
		/**Barst処理*/
		protected void setBrast(ItemStack pItemstack) {
			checkTags(pItemstack);
		...
		/**Barst処理*/
		protected boolean cycleBrast(ItemStack pItemstack) {
			checkTags(pItemstack);
			NBTTagCompound lnbt = pItemstack.getTagCompound();
		...
		/**Barst処理*/
		protected void resetBrast(ItemStack pItemstack) {
			checkTags(pItemstack);
	    ...
	    {
	    	/*if(iii == 1){
	    		return stack != null && stack.getItem() == mod_GVCR.gun_magazinehg;
	    	}else if(iii == 2){
	    		return stack != null && stack.getItem() == mod_GVCR.gun_magazinemg;
	    	}else if(iii == 3){
	    		return stack != null && stack.getItem() == mod_GVCR.gun_magazinerpg;
	    	}else{
	    		return stack != null && stack.getItem() == mod_GVCR.gun_magazine;
	    	}*/
	    	String tab = null;
	    	boolean re = false;
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		...
		if(this instanceof IGun_Sword) {
			...
			String recoilads = String.valueOf(this.recoil_ads);
			String retime = String.valueOf(this.reloadtime);
			String nokori = String.valueOf(getMaxDamage() - stack.getItemDamage());
    ...
    public void onUpdate(ItemStack itemstack, World world, Entity entity, int i, boolean flag)
    {
    	...
    	if(flag) {
    		...
    		if(this.underbarrel.hasTagCompound()) {
				this.gunbase_reload(this.underbarrel, world, entity, i, flag);
			}
    	}*/
    		if(!world.isRemote) {
    ...
    public void Fire_AR(ItemStack itemstack, World world, Entity entity, int i, boolean flag) {
		...
		{
    		if(itemstack == entityplayer.getHeldItemOffhand() && itemstack.getItem() instanceof ItemGunBase) {
    			...
    			if (lflag && lflag_barst && left) {
					if (li > 0) {
						this.Fire_Base(itemstack, world, entityplayer, li, false, nbt);
    			...
    			if (entityplayer.getActiveItemStack() == itemstack) {
    				if (flag)
    				{
    					if (lflag && lflag_barst) {
    						if (li > 0) {
    							this.Fire_Base(itemstack, world, entityplayer, li, true, nbt);
    						} else {
    							ItemGunBase.updateCheckinghSlot(entity, itemstack);
    						}
    					} else {
    ...
    public void Fire_SR(ItemStack itemstack, World world, Entity entity, int i, boolean flag) {
    	int li = getMaxDamage() - itemstack.getItemDamage();
    	EntityPlayer entityplayer = (EntityPlayer) entity;
    	boolean lflag = cycleBolt(itemstack);
		boolean lflag_barst = cycleBrast(itemstack);
    ...
    public void Fire_Base(ItemStack itemstack, World world, EntityPlayer entityplayer, int i, boolean hand, NBTTagCompound nbt) {
    	
    	if(!world.isRemote) {
    		FireBullet(itemstack, world, entityplayer);
    		resetBolt(itemstack);
    		if(canbarst)setBrast(itemstack);
    		//recoilreBolt(itemstack);
    		ItemGunBase.updateCheckinghSlot(entityplayer, itemstack);
    		...
    		boolean flag = false;
    		{
    			...
    			if(!off.isEmpty()) {
    				if(off.getItem() instanceof ItemGunBase) {
    					ItemGunBase gun_off = (ItemGunBase) off.getItem();
    					if(!gun_off.can_dual_hand) {
    						flag = true;
    					}
    				}
    				if(main.getItem() instanceof ItemGunBase) {
    					ItemGunBase gun_main = (ItemGunBase) main.getItem();
    					if(!gun_main.can_dual_hand) {
    						flag = true;
    					}
    				}
    			}
    			...
    			if(itemstack.hasTagCompound()) {
    				...
    				if (entityplayer.isSneaking()) {
    					entityplayer.rotationPitch += (itemRand.nextFloat() * -this.recoil_ads) * rrecoil;
    				} else {
    					entityplayer.rotationPitch += (itemRand.nextFloat() * -2F) * rrecoil;
    				}
    				if(FMLCommonHandler.instance().getMinecraftServerInstance() != null) {
    					for (EntityPlayerMP playermp : FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().getPlayers())
    						GVCLPacketHandler.INSTANCE2.sendTo(new GVCLClientMessageKeyPressed(2, entityplayer.getEntityId(), entityplayer.rotationPitch), playermp);
    				}
    			}
			...
			if(itemstack.hasTagCompound()) {
				//if(world.isRemote) 
				{
				if(world.isRemote)nbt.setBoolean("Recoiled", false);
    ...
    /*public void Fire_AR(ItemStack itemstack, World world, Entity entity, int i, boolean flag) {
		...
		boolean lflag_barst = cycleBrast(itemstack);
		NBTTagCompound nbt = itemstack.getTagCompound();
    	if (entity != null && !world.isRemote && entityplayer != null) 
		{
			if (entityplayer.getActiveItemStack() == itemstack)
			if (flag)
			{
					...
					if (li > 0) {
						...
						ItemGunBase.updateCheckinghSlot(entity, itemstack);
						{
							if(itemstack.hasTagCompound()) {
								...
								GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2, entityplayer.getEntityId(), entityplayer.rotationPitch));
								nbt.setBoolean("Recoiled", false);
								GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(3, entityplayer.getEntityId(), false));
							}
						}
					} else {
						ItemGunBase.updateCheckinghSlot(entity, itemstack);
				...
				if (cocking) {
					if (li > 0) {
						...
						nbt.setBoolean("Cocking", false);
						{
							...
							GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2, entityplayer.getEntityId(), entityplayer.rotationPitch));
							GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(3, entityplayer.getEntityId(), false));
							GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(4, entityplayer.getEntityId(), false));
						}
					} else {
						ItemGunBase.updateCheckinghSlot(entity, itemstack);
					}
    ...
    public void FireBullet(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
		...
		//par2World.playSound((EntityPlayer) null, par3EntityPlayer.posX, par3EntityPlayer.posY, par3EntityPlayer.posZ,
		//		SoundEvent.REGISTRY.getObject(new ResourceLocation(modid, sound)), SoundCategory.NEUTRAL, 4.0F, this.soundsp);//1.0,1.2
		if(!this.mugen) {
			{
		...
		
		{
			for(int p = 0; p < pellet; ++p){
				...
				var8.missile_targeting_time_start = this.missile_targeting_time_start;
				var8.missile_targeting_time_end = this.missile_targeting_time_end;
				var8.missile_targeting_size = this.missile_targeting_size;
				
				var8.exlevel = ex_leve;
				var8.P_ID = p__id;
				...
				if(par1ItemStack.hasTagCompound()) {
					...
					if(am_grip) {
						...
						if (par3EntityPlayer.isSneaking()) {
							bbure = nbt_bure_ads * this.bure_ads;
						}else {
							bbure = nbt_bure;
						}
					}else {
						if (par3EntityPlayer.isSneaking()) {
							bbure = this.bure * this.bure_ads;
						}
					}
				}else {
					if (par3EntityPlayer.isSneaking()) {
				...
				if(nbt.getBoolean("am_bullet")){
					bbure = bbure * nbt.getFloat("bure_bullet");
				}
		...
		/*if (par3EntityPlayer.isSneaking()) {
			par3EntityPlayer.rotationPitch += (itemRand.nextFloat() * -this.recoil_ads) * this.recoil;
		} else {
			par3EntityPlayer.rotationPitch += (itemRand.nextFloat() * -2F) * this.recoil;
		}*/
	}
    	...
    	if(entity instanceof EntityPlayer)
    	{
    		...
    		if(this.gun_can_get_underbarrel && (!this.underbarrel.isEmpty() && this.underbarrel.getItem() instanceof ItemGunBase)) {
				...
				if (!par2World.isRemote && this.underbarrel.getItemDamage() != under.getMaxDamage()) {
					...
					inventory.setInventorySlotContents(4, this.underbarrel);
					/*ItemStack i1 = inventory.getStackInSlot(1);
					if(!i1.isEmpty())inventory.setInventorySlotContents(1, i1);
					ItemStack i2 = inventory.getStackInSlot(2);
					if(!i2.isEmpty())inventory.setInventorySlotContents(2, i2);
					ItemStack i3 = inventory.getStackInSlot(3);
					if(!i3.isEmpty())inventory.setInventorySlotContents(3, i3);
					ItemStack i5 = inventory.getStackInSlot(5);
					if(!i5.isEmpty())inventory.setInventorySlotContents(5, i5);
					*/
					ItemStack i1 = this.sight_item;
					if(!i1.isEmpty())inventory.setInventorySlotContents(1, i1);
    ...
    /**GVCEventsGunRender_Firstでtrueに返すようにしたので削除*/
 ...
 public int pelletbase;
    ...
    public void Attachment(ItemGunBase gun, ItemStack itemstack, World world, EntityPlayer entityplayer, int i, boolean flag) {
    	...
    	/*{
    		...
    		if(!this.can_dual_hand) {
        		if(itemstack == entityplayer.getHeldItemOffhand()) {
        			this.recoil = this.recoilbase * 5;
        			this.bure = this.burebase * 5;
            		//if(!entityplayer.getHeldItemMainhand().isEmpty() && entityplayer.getHeldItemMainhand().getItem() instanceof ItemGunBase) {
            			//ItemGunBase gun_main = (ItemGunBase) entityplayer.getHeldItemMainhand().getItem();
            		//}
            	}
        		if(itemstack == entityplayer.getHeldItemMainhand()) {
