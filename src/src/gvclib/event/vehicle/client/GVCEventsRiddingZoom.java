...
public class GVCEventsRiddingZoom {
    ...
    @SubscribeEvent
	  public void rendertest___(GuiScreenEvent.DrawScreenEvent.Post event)
	  {
		Minecraft minecraft = Minecraft.getMinecraft();
	...
	/*@SideOnly(Side.CLIENT)
    @SubscribeEvent
	  public void rendertest___(RenderGameOverlayEvent.Text event)
	  {
		Minecraft minecraft = Minecraft.getMinecraft();
		EntityPlayer entityplayer = minecraft.player;
		...
		minecraft.getTextureManager().bindTexture(new ResourceLocation("gvclib:textures/misc/night.png"));
			...
			if(minecraft.gameSettings.thirdPersonView == 1){
				...
				GL11.glTranslatef(0, -1, -2);
					...
					if(balaam.getFTMode() != 1){
						...
						//GL11.glTranslatef(balaam.ridding_view_x, balaam.ridding_view_y  * (float)Math.cos(f1), balaam.ridding_view_z * (float)Math.cos(f1));//24/02/17とりあえず削除
        				...
        				if (llist != null) {
        					for (int lj = 0; lj < llist.size(); lj++) {
        						...
        						if(entity1 instanceof EntityB_Missile) {
        							this.renderRadarEntity(minecraft, scaledresolution, balaam, entity1, "gvclib:textures/hud/enemy_missile.png");
        						}
        					}
        				}
        			}
