    	...
    	if (entity.canBeSteered() && entity.getControllingPassenger() != null && entity.getHealth() > 0.0F)
		{
			...
			if(entity.weapon4true && entity.getArmID_S() == 2) {
				if (entity.getFTMode() == 1) {
					entity.spg_mode = true;
					{
						...
						float xx = 0;
						float zz = 0;
						if (entity.getMoveForward_PL() > 0.0F && (entity.spg_yaw < range && entity.spg_picth < range)) {
							xx = xx + 1;
						}
						if (entity.getMoveForward_PL() < 0.0F && (entity.spg_yaw > -range && entity.spg_picth > -range)) {
							xx = xx - 1;
						}
						if (entity.getMoveStrafing_PL() > 0.0F && (entity.spg_yaw < range && entity.spg_picth < range)) {
							zz = zz + 1;
						}
						if (entity.getMoveStrafing_PL() < 0.0F && (entity.spg_yaw > -range && entity.spg_picth > -range)) {
							zz = zz - 1;
			...
			/*if(AI_EntityWeapon.getRange(entity, entity.mob_w2range, entity.mob_w2range_max_y, entity.mob_w2range_min_y)){
				...
				if(entitylivingbase.getOwner() != null) {
					AI_EntityMove_Tank_Squad.moveBoat(entity, entitylivingbase, f1, entity.sp, entity.turnspeed, entity.mob_min_range, entity.mob_max_range);
				}else {
					AI_EntityMoveTank.moveBoat(entity, entitylivingbase, f1, entity.sp, entity.turnspeed, entity.mob_min_range, entity.mob_max_range);
				}
				...
				if(AI_EntityWeapon.getRange_new(entitylivingbase, entity, entity.mob_w2range, entity.mob_w2range_max_y, entity.mob_w2range_min_y)){
					if(entity.cooltime2 >= entity.ammo2 && entity.getRemain_R() > 0){
						entity.counter2 = true;
						entity.cooltime2 = 0;
					}
