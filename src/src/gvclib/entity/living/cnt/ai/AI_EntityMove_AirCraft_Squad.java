...
public class AI_EntityMove_AirCraft_Squad {
	public static void movefighter(EntityGVCLivingBase entity, EntityGVCLivingBase ridding, float f1, float sp,
		...
		if (!ridding.getTargetEntity_isLiving() || owner_range(ridding, 80)){
			boolean flags = true;
			float f11 = 0;
			...
			if (llist != null) {
				...
				for (int lj = 0; lj < llist.size(); lj++) {
					...
					if (entity1.canBeCollidedWith()) {
						...
						if (entity1 != null && entity1 instanceof ISoldier && ridding.getMoveT() != 3) {
						}
					}
				}
				//player
...
//////////////////////////////////////////////////////////////////////////
			float f11 = 0;
			if(tagetentity != null && ridding.getMoveT() != 3 && ridding.getMoveT() != 3) {
			...
			if (entity.getHealth() > 0.0F && ridding.getMoveT() == 3) {
				movestay(entity, ridding, turnspeed, turnspeed, turnspeed, max, range);
			}
		}
	...
	public static boolean owner_range(EntityGVCLivingBase ridding, int range) {
		boolean flag = true;
		float f11 = 0;
		double x = ridding.posX;
		double y = ridding.posY;
		...
		AxisAlignedBB axisalignedbb2 = (new AxisAlignedBB((double) (x - han), (double) (y - han),
				(double) (z - han), (double) (x + han), (double) (y + han), (double) (z + han)))
						.grow(han);
		List<Entity> llist = ridding.world.getEntitiesWithinAABBExcludingEntity(ridding, axisalignedbb2);
		if (llist != null) {
			//player
			for (int lj = 0; lj < llist.size(); lj++) {
				Entity entity1 = (Entity) llist.get(lj);
