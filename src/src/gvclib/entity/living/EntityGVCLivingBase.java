...
public abstract class EntityGVCLivingBase extends EntityCreature
{
	...
	//public boolean sneak;//25/02/02試験的に削除
	...
	/**mob_w1range_max_y等が未機能になっていたので実装
	 *  trueでAI_EntityWeapon.getRange_newで適応*/
	public boolean mob_attackrange_hight = false;
	...
	/*private static final DataParameter<Float> positionX = EntityDataManager.<Float>createKey(EntityGVCLivingBase.class, DataSerializers.FLOAT);
	private static final DataParameter<Float> positionY = EntityDataManager.<Float>createKey(EntityGVCLivingBase.class, DataSerializers.FLOAT);
	private static final DataParameter<Float> positionZ = EntityDataManager.<Float>createKey(EntityGVCLivingBase.class, DataSerializers.FLOAT);*/
    ...
    public static final DataParameter<Integer> aitype = 
    		EntityDataManager.<Integer>createKey(EntityGVCLivingBase.class, DataSerializers.VARINT);
    public static final DataParameter<Integer> aitype2 = 
    		EntityDataManager.<Integer>createKey(EntityGVCLivingBase.class, DataSerializers.VARINT);
    public static final DataParameter<Integer> aitype3 = 
    		EntityDataManager.<Integer>createKey(EntityGVCLivingBase.class, DataSerializers.VARINT);
	...
	= EntityDataManager.<Optional<UUID>>createKey(EntityGVCLivingBase.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	 ...
	 private static final DataParameter<Float> MoveForward_PL = 
	    		EntityDataManager.<Float>createKey(EntityGVCLivingBase.class, DataSerializers.FLOAT);
	 
	 private static final DataParameter<Float> MoveStrafing_PL = 
	    		EntityDataManager.<Float>createKey(EntityGVCLivingBase.class, DataSerializers.FLOAT);
...
//        this.ignoreFrustumCheck=true;
    ...
    public void writeEntityToNBT(NBTTagCompound compound)
    {
        ...
        /*compound.setFloat("positionX", getPositionX());
        compound.setFloat("positionY", getPositionY());
        compound.setFloat("positionZ", getPositionZ());*/
   ...
   //     compound.setBoolean("sneak_gvc", getSneak());
        ...
        compound.setFloat("MoveForward_PL", getMoveForward_PL());
        compound.setFloat("MoveStrafing_PL", getMoveStrafing_PL());
    ...
    public void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        /*this.setPositionX(compound.getFloat("positionX"));
        this.setPositionY(compound.getFloat("positionY"));
        this.setPositionZ(compound.getFloat("positionZ"));*/
  ...
  //      this.setMasterID(compound.getInteger("masterid"));
        ...
        this.setMoveForward_PL(compound.getFloat("MoveForward_PL"));
        this.setMoveStrafing_PL(compound.getFloat("MoveStrafing_PL"));
    }
    ...
    protected void entityInit()
    {
       ...
       /* this.dataManager.register(positionX, Float.valueOf(0));
        this.dataManager.register(positionY, Float.valueOf(0));
        this.dataManager.register(positionZ, Float.valueOf(0));*/
        ...
        this.dataManager.register(MoveForward_PL, Float.valueOf(0F));
        this.dataManager.register(MoveStrafing_PL, Float.valueOf(0F));
    }
    ...
    public float getPositionX() {
		return ((this.dataManager.get(positionX)).intValue());
	}
    public void setPositionX(float stack) {
	this.dataManager.set(positionX, Float.valueOf(stack));
    }
    public float getPositionY() {
		return ((this.dataManager.get(positionY)).intValue());
	}
    public void setPositionY(float stack) {
	this.dataManager.set(positionY, Float.valueOf(stack));
    }
    public float getPositionZ() {
		return ((this.dataManager.get(positionZ)).intValue());
	}
    public void setPositionZ(float stack) {
	this.dataManager.set(positionZ, Float.valueOf(stack));
    }
    ...
    */
    	...
    	if(!this.world.isRemote){
			if(FMLCommonHandler.instance().getMinecraftServerInstance() != null) {
				for (EntityPlayerMP playermp : FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().getPlayers()) {
					GVCLPacketHandler.INSTANCE2.sendTo(new GVCLClientMessageKeyPressed(3301, this.getEntityId(), stack), playermp);//this.targetentity
				}
    		}
		}
		if(this.world.isRemote){
			GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(3301, this.getEntityId(), stack));
		}
    }
    /**aitypetime2::経過時間
    ...
    public void setAIType2(int stack) {
    	...
    	if(!this.world.isRemote){
			if(FMLCommonHandler.instance().getMinecraftServerInstance() != null) {
				for (EntityPlayerMP playermp : FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().getPlayers()) {
					GVCLPacketHandler.INSTANCE2.sendTo(new GVCLClientMessageKeyPressed(3302, this.getEntityId(), stack), playermp);//this.targetentity
				}
    		}
		}
		if(this.world.isRemote){
			GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(3302, this.getEntityId(), stack));
		}
    }
    /*** 自動では変更されない*/
    ...
    public void setAIType3(int stack) {
    	...
    	if(!this.world.isRemote){
			if(FMLCommonHandler.instance().getMinecraftServerInstance() != null) {
				for (EntityPlayerMP playermp : FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().getPlayers()) {
					GVCLPacketHandler.INSTANCE2.sendTo(new GVCLClientMessageKeyPressed(3303, this.getEntityId(), stack), playermp);//this.targetentity
				}
    		}
		}
		if(this.world.isRemote){
			GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(3303, this.getEntityId(), stack));
		}
    }
	...
	public boolean getattacktask() {
		...
		if(this.targetentity != null) {
			if(this.targetentity.isDead || this.targetentity.getHealth() <= 0) {
				flag = false;
	...
	public void setattacktask(boolean stack) {
		if(!stack) {
			...
			if(this.world.isRemote){
				GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(202, this.getEntityId()));
			}
		}else {
			this.dataManager.set(attacktask, Boolean.valueOf(stack));
			...
			if(this.world.isRemote && this.targetentity != null){
				GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(203, this.getEntityId(), this.targetentity.getEntityId()));
			}
		}
	}
	...
	/**GVCEventsRiddingVehicle_Updateで実装*/
    public float getMoveForward_PL() {
    	return ((this.dataManager.get(MoveForward_PL)).floatValue());
    }
    /**GVCEventsRiddingVehicle_Updateで実装*/
    public void setMoveForward_PL(float stack) {
    	this.dataManager.set(MoveForward_PL, Float.valueOf(stack));
    }
    /**GVCEventsRiddingVehicle_Updateで実装*/
    public float getMoveStrafing_PL() {
    	return ((this.dataManager.get(MoveStrafing_PL)).floatValue());
    }
    /**GVCEventsRiddingVehicle_Updateで実装*/
    public void setMoveStrafing_PL(float stack) {
    	this.dataManager.set(MoveStrafing_PL, Float.valueOf(stack));
    }
    ...
    public boolean trueontick = false;
    public int ontick = 0;
    ...
    public boolean aitype3_auto = false;
    
    public boolean cooltime_auto = true;
    ...
    public void onUpdate() {
    	...
    	if(trueontick)ontick_counter();
    	
    	if(cooltime_auto)cooltime_counter();
    ...
    public void cooltime_counter() {
    	 {
    		/*GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2000, this.getEntityId(), startuptime));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2001, this.getEntityId(), cooltime));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2002, this.getEntityId(), cooltime2));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2003, this.getEntityId(), cooltime3));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2004, this.getEntityId(), cooltime4));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2005, this.getEntityId(), cooltime5));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2006, this.getEntityId(), cooltime6));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2007, this.getEntityId(), gun_count1));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2008, this.getEntityId(), gun_count2));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2009, this.getEntityId(), gun_count3));
    		GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(2010, this.getEntityId(), gun_count4));
    		
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2000, this.getEntityId(), startuptime));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2001, this.getEntityId(), cooltime));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2002, this.getEntityId(), cooltime2));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2003, this.getEntityId(), cooltime3));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2004, this.getEntityId(), cooltime4));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2005, this.getEntityId(), cooltime5));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2006, this.getEntityId(), cooltime6));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2007, this.getEntityId(), gun_count1));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2008, this.getEntityId(), gun_count2));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2009, this.getEntityId(), gun_count3));
    		GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(2010, this.getEntityId(), gun_count4));*/
    ...
    public void AIType_3() {
    	if(this.aitype3_auto) {
    ...
    public void targetValidtime() {
    	...
    	if(this.getattacktask())//if(!this.world.isRemote && (this.getattacktask() || this.getMobMode() == 1))
    	{
    		/*//多分いらない2021/2/12
    		if(FMLCommonHandler.instance().getMinecraftServerInstance() != null) {
				for (EntityPlayerMP playermp : FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().getPlayers()) {
					GVCLPacketHandler.INSTANCE2.sendTo(new GVCLClientMessageKeyPressed(201, this.getEntityId()), playermp);//base.setattacktask(true);
				}
    		}*/
			...
			if(this.targetentity != null) {
				if(this.targetentity.getHealth() <= 0F || this.targetentity.isDead) {
					this.target = false;
					this.targetentity = null;
					this.mitargettime = 0;
					this.mitarget = null;
