...
public abstract class EntityVehicleBase extends EntityGVCLivingBase implements IAnimals
{
	...
	/*
	public double riddingx2 = 0;
	public double riddingy2 = 0;
	public double riddingz2 = 0;
	
	public double riddingx_roteplayer2 = 0;
	public double riddingy_roteplayer2 = 0;
	public double riddingz_roteplayer2 = 0;
	*/
	...
	public int[] missile_targeting_time_start1 = new int[32];
	public int[] missile_targeting_time_end1 = new int[32];
	public float[] missile_targeting_size1 = new float[32];
	...
	public int[] missile_targeting_time_start11 = new int[32];
	public int[] missile_targeting_time_end11 = new int[32];
	public float[] missile_targeting_size11 = new float[32];
	...
	public int[] missile_targeting_time_start12 = new int[32];
	public int[] missile_targeting_time_end12 = new int[32];
	public float[] missile_targeting_size12 = new float[32];
	...
	public int[] missile_targeting_time_start2 = new int[32];
	public int[] missile_targeting_time_end2 = new int[32];
	public float[] missile_targeting_size2 = new float[32];
	...
	public int[] missile_targeting_time_start3 = new int[32];
	public int[] missile_targeting_time_end3 = new int[32];
	public float[] missile_targeting_size3 = new float[32];
	...
	public int[] missile_targeting_time_start4 = new int[32];
	public int[] missile_targeting_time_end4 = new int[32];
	public float[] missile_targeting_size4 = new float[32];
	 ...
	 * 　　  主に座席0.0.0の航空機に使用
	 * */
	public boolean vehicle_model_onground = false;
	public double vehicle_model_onground_x = 0.0D;
	public double vehicle_model_onground_y = 0.0D;
	public double vehicle_model_onground_z = 0.0D;
	
	/**240224追加_ヘリボーン用のカウンタ/AI_EntityMove_HeliBorneで使用*/
    ...
    public void onLivingUpdate()
    {
        ...
        if(mod_GVCLib.cfg_vehicle_death_fire){
        	if(deathTicks > 0) {
        		this.setFire(10);
        	}
        }
        
        {
        	if (this.canBeSteered() && this.getControllingPassenger() != null && this.getHealth() > 0.0F)
			{
				...
				{
					EntityPlayer player2 = (EntityPlayer) this.getControllingPassenger();
					{
						this.setMoveForward_PL(player2.moveForward);
						this.setMoveStrafing_PL(player2.moveStrafing);
						if(player2.moveForward != 0 || player2.moveStrafing != 0)
						{
							if(!this.world.isRemote) {
								 if(FMLCommonHandler.instance().getMinecraftServerInstance() != null) {
										for (EntityPlayerMP playermp : FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().getPlayers()) {
											GVCLPacketHandler.INSTANCE2.sendTo(new GVCLClientMessageKeyPressed(600, this.getEntityId(), player2.moveForward, player2.moveStrafing, 0), playermp);
										}
									}
							}else {
								GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(600, this.getEntityId(), player2.moveForward, player2.moveStrafing, 0));
							}
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand)
    {
        ...
        if(this.getControllingPassenger() != null && this.getControllingPassenger() instanceof EntityGVCLivingBase) {
        	...
        	if(!(ridding instanceof IMob)) {
        		if(flag && itemstack.getItem() instanceof ItemWrench) {
	...
	//			float f22 = this.rotationPitch * (2 * (float) Math.PI / 360);
				...
				*/
				...
				if (i == 0) {
					if(vehicle_model_onground && this.onGround) {
						passenger.setPosition(this.posX + vec3d.x + vehicle_model_onground_x,this.posY + vec3d.y + vehicle_model_onground_y, this.posZ + vec3d.z + vehicle_model_onground_z);
					}else {
						passenger.setPosition(this.posX + vec3d.x,this.posY + vec3d.y, this.posZ + vec3d.z);
					}
				}else {
					if(mod_GVCLib.cfg_vehicle_server_client_async) {
						if(!this.world.isRemote){
							//GVCLPacketHandler.INSTANCE.sendToServer(new GVCLMessageKeyPressed(548, passenger.getEntityId(), 
							//		(float)(this.posX + vec3d.x),(float)(this.posY + vec3d.y), (float)(this.posZ + vec3d.z), 0));
				...
				/*{
					int ixx = MathHelper.floor(passenger.posX / 16.0D);
			        int jxx = MathHelper.floor(passenger.posZ / 16.0D);
			        
			        int iyy = MathHelper.floor(this.posX / 16.0D);
			        int jyy = MathHelper.floor(this.posZ / 16.0D);

			        if (ixx != iyy || jxx != jyy)
			        {
			        	passenger.posX = this.posX;
			        	passenger.posZ = this.posZ;
    ...
    public void weapon1active(Vec3d looked, EntityPlayer entitylivingbase) {
    	if(this.counter1 && gun_count1 >= w1cycle && this.getRemain_L() > 0){
			...
			for(int w1 = 0; w1 < weapon1; ++w1) {
				...
				we.missile_targeting_time_start = this.missile_targeting_time_start1[w1];
				we.missile_targeting_time_end = this.missile_targeting_time_end1[w1];
				we.missile_targeting_size = this.missile_targeting_size1[w1];
				we.dameid = 0;
    ...
    public void weapon11active(Vec3d looked, EntityPlayer entitylivingbase) {
    	if(this.counter1 && gun_count1 >= w1cycle && this.getRemain_L() > 0){
			...
			for(int w11 = 0; w11 < weapon11; ++w11) {
				...
				/*PL_Weapon.WeaponAttack(this, this.bullet_type11[w11], entitylivingbase, 
						this.bullet_model11[w11],this.bullet_tex11[w11],this.fire_model11[w11],this.fire_tex11[w11], this.fire_time11[w11],
						SoundEvent.REGISTRY.getObject(new ResourceLocation(modid, this.sound_fire11[w11])), 
						side, this.fire_pointx11[w11], this.fire_pointy11[w11], this.fire_pointz11[w11], this.fire_yoffset11[w11],
						x, this.posY, z, looked, rotefire,0,0,
						this.bullet_damage11[w11], this.bullet_speed11[w11], this.bullet_bure11[w11], 
						this.bullet_expower11[w11], this.bullet_ex11[w11], 
						this.bullet_kazu11[w11], this.bullet_gravity11[w11], this.bullet_livingtime11[w11], 0);*/
				PL_Weapon_new we = new PL_Weapon_new(this, entitylivingbase);
				...
				we.missile_targeting_time_start = this.missile_targeting_time_start11[w11];
				we.missile_targeting_time_end = this.missile_targeting_time_end11[w11];
				we.missile_targeting_size = this.missile_targeting_size11[w11];
				we.dameid = 0;
				float f = Math.abs(rotefire) - Math.abs(this.rotationYaw);
				if(f <= this.w1kakumax && f >= this.w1kakumin) {
					boolean canfire = true;
    ...
    public void weapon12active(Vec3d looked, EntityPlayer entitylivingbase) {
    	if(this.counter1 && gun_count1 >= w1cycle && this.getRemain_L() > 0){
			...
			for(int w12 = 0; w12 < weapon12; ++w12) {
				...
				we.missile_targeting_time_start = this.missile_targeting_time_start12[w12];
				we.missile_targeting_time_end = this.missile_targeting_time_end12[w12];
				we.missile_targeting_size = this.missile_targeting_size12[w12];
				we.dameid = 0;
				float f = Math.abs(rotefire) - Math.abs(this.rotationYaw);
				if(f <= this.w1kakumax && f >= this.w1kakumin) {
					boolean canfire = true;
    ...
    public void weapon2active(Vec3d looked, EntityPlayer entitylivingbase) {
    	if(this.counter2 && gun_count2 >= w2cycle && this.getRemain_R() > 0){
			...
			for(int w2 = 0; w2 < weapon2; ++w2) {
				...
				we.missile_targeting_time_start = this.missile_targeting_time_start2[w2];
				we.missile_targeting_time_end = this.missile_targeting_time_end2[w2];
				we.missile_targeting_size = this.missile_targeting_size2[w2];
				we.dameid = 0;
				float f = Math.abs(rotefire) - Math.abs(this.rotationYaw);
    ...
    public void weapon3active(Vec3d looked, EntityPlayer entitylivingbase) {
    	if(this.counter3 && gun_count3 >= w3cycle && this.getRemain_A() > 0){
			...
			for(int w3 = 0; w3 < weapon3; ++w3) {
				...
				we.missile_targeting_time_start = this.missile_targeting_time_start3[w3];
				we.missile_targeting_time_end = this.missile_targeting_time_end3[w3];
				we.missile_targeting_size = this.missile_targeting_size3[w3];
				we.dameid = 0;
				float f = Math.abs(rotefire) - Math.abs(this.rotationYaw);
    ...
    public void weapon4active(Vec3d looked, EntityPlayer entitylivingbase) {
    	if(this.counter4 && gun_count4 >= w4cycle && this.getRemain_S() > 0){
			...
			for(int w4 = 0; w4 < weapon4; ++w4) {
				...
				we.missile_targeting_time_start = this.missile_targeting_time_start4[w4];
				we.missile_targeting_time_end = this.missile_targeting_time_end4[w4];
				we.missile_targeting_size = this.missile_targeting_size4[w4];
				we.dameid = 0;
				float f = Math.abs(rotefire) - Math.abs(this.rotationYaw);
