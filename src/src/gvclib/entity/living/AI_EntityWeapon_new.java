...
public class AI_EntityWeapon_new {
	...
	public void Attacktask_LivingBase(EntityLivingBase entity, EntityGVCLivingBase onride, double range){
		...
		AxisAlignedBB axisalignedbb = (new AxisAlignedBB((double) (k - han), (double) (l - miny),(double) (i - han), 
				(double) (k + han), (double) (l + maxy), (double) (i + han)))
						.grow(1);
		List<Entity> llist = entity.world.getEntitiesWithinAABBExcludingEntity(entity,axisalignedbb);
		if(onride.getMoveT() == 5 || (onride.getMoveT() == 1 && onride.getAIType() == 1)) {
			double x1 = onride.getMoveX();
			double y1 = onride.getMoveY();
			double z1 = onride.getMoveZ();
			double dyp = onride.posY + maxy;
			double dym = onride.posY - miny;
			if (dyp > y1 && dym < y1) {
				double d5 = x1 - onride.posX;
	...
	{
		if(target == null) {
			if(player.getMoveT() == 5  || (player.getMoveT() == 1 && player.getAIType() == 1)) {
				if(!player.world.isRemote){
					EntityGVC_Dummy particle = new EntityGVC_Dummy(player.world);
					particle.setLocationAndAngles(player.getMoveX(), player.getMoveY(), player.getMoveZ(), 0.0F, 0.0F);
					player.world.spawnEntity(particle);
					target = particle;
				}
			}
		}
		if(missile_targeting_time_start <= 0) {
			missile_targeting_time_start = 5;
		}
		if(missile_targeting_time_end <= 0) {
			missile_targeting_time_end = 80;
		}
		if(missile_targeting_size <= 0) {
			missile_targeting_size = 2.0F;
		}
		...
		if(id < 10  || id == 31){
			for (int ka = 0; ka < kazu; ++ka) {
				...
				var3.muteki = damegetime;
				 ...
				 var3.missile_targeting_time_start = missile_targeting_time_start;
				var3.missile_targeting_time_end = missile_targeting_time_end;
				var3.missile_targeting_size = missile_targeting_size;
				 var3.P_ID = P_ID;
				 var3.P_TIME = P_TIME;
