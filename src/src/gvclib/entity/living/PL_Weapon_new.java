		...
		*/
		...
		if(id < 13){
			/*if(!player.world.isRemote) {
	            ...
	            entitysnowball.setLocationAndAngles(px + xx11  - offx, py + h + kaku, pz + zz11  - offz, player.rotationYaw,
						player.rotationPitch);
	            //entitysnowball.setThrowableHeading(lock.x, lock.y + 0.03 + yoffset, lock.z, speed, bure);
			...
			for (int ka = 0; ka < kazu; ++ka) {
				EntityBBase var3;
				{
					...
					//var3.setThrowableHeading(lock.x, lock.y + 0.03 + yoffset, lock.z, speed, bure);
					//var3.setHeadingFromThrower(player, vehicle.rotationPitch + 0.03F, vehicle.rotationYaw, 0.0F,speed, bure);
					if (!player.world.isRemote) {
