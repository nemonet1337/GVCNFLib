...
public class EntityB_Missile extends EntityB_Shell {
    ...
    {
        super.onUpdate();
//        if(this.mitarget == null)System.out.println("lockon_null");
        this.world.spawnParticle(EnumParticleTypes.CLOUD, this.posX, this.posY + 0D, this.posZ, 0.0D, 0.0D, 0.0D, new int[0]);
        	
        if(time > 80){
        	this.mitarget = null;
        }
        if(time < 5 && this.mitarget != null)GVCLPacketHandler.INSTANCE2.sendToAll(new GVCLClientMessageKeyPressed(325, this.getEntityId(), this.mitarget.getEntityId()));
        
        if(autoaim) {
        	if(this.mitargetid != -1 && time > missile_targeting_time_start) 
        	{
...
//        			System.out.println("lockonID");
        		...
        		if (this.mitarget != null) {
        			if (this.mitarget.getRidingEntity() instanceof EntityVehicleBase && this.mitarget.getRidingEntity() != null) {// 1
        				EntityVehicleBase vehicle = (EntityVehicleBase) this.mitarget.getRidingEntity();
        				if(vehicle.width < this.missile_targeting_size && vehicle.height < this.missile_targeting_size) {
            				this.mitarget = null;
            			}
        			}else {
        				if(this.mitarget.width < this.missile_targeting_size && this.mitarget.height < this.missile_targeting_size) {
            				this.mitarget = null;
            			}
        			}
        		}
                ...
                for (int lj = 0; lj < llist.size(); lj++) {
                	...
                	if (entity1.canBeCollidedWith() && (entity1 != entitylivingbase))
                    {
                				...
                				&& entity1 != null)
                        {
                            ...
                            {
                				if(time > missile_targeting_time_start && time < missile_targeting_time_end){
 ...
 //   										System.out.println("lockon");
        	...
        	if(this.getThrower() != null && time > 2) {
        		EntityLivingBase entitylivingbase = this.getThrower();
            ...
            if(llist!=null){
                for (int lj = 0; lj < llist.size(); lj++) {
                    ...
                    {
                		if (entity1 != null &&entity1 == entitylivingbase)
                        {
                			Vec3d lock = entitylivingbase.getLookVec();
...
//		entity1.addPotionEffect(new PotionEffect(MobEffects.GLOWING, 10, 10));
        ...
        double d7 = (entity1.posY + (double) entity1.height) - this.posY;//double d7 = (entity1.posY + (double) entity1.height/2 + 0.3D) - this.posY;
	...
	/*test
	{
		...
		if(time > 0){
			double d6 = entity1.posX - this.posX;
	        double d7 = entity1.posY - this.posY;
	        double d4 = entity1.posZ - this.posZ;
		...
		else if(en.isPotionActive(MobEffects.GLOWING))
		 { 
			double d6 = entity1.posX - this.posX;
	        double d7 = entity1.posY - this.posY;
	        double d4 = entity1.posZ - this.posZ;
