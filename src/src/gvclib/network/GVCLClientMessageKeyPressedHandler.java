    ...
    public IMessage onMessage(GVCLClientMessageKeyPressed message, MessageContext ctx) {
      ...
      //受け取ったMessageクラスのkey変数の数字をチャットに出力
        ...
        if(message.key == 11){
    		...
    		{
    			NBTTagCompound nbt = player.getEntityData();
    			nbt.setInteger("hitentity_headshot", 20);
    			{
    				//player.playSound(SoundEvents.ENTITY_ARROW_HIT, 1.0F, 1.2F);
    			}
    		}
        ...
        if(message.key == 325){
          ...
          try {
              if(world != null){
                  ...
                  Entity target = world.getEntityByID(message.da);
                  if(target != null && tgtEntity != null && tgtEntity instanceof EntityBBase && target instanceof EntityLivingBase) {
                	  EntityBBase bullet = (EntityBBase) tgtEntity;
                	  bullet.mitarget = (EntityLivingBase) target;
                  }
              }
          ...
          try {
              if(world != null){
                  ...
                  if(message.data != null && tgtEntity != null && tgtEntity instanceof EntityBBase) {
                      ...
                      EntityBBase bullet = (EntityBBase) tgtEntity;
                      if(bullet.startposX == 0 && bullet.startposZ == 0) {
                    	  bullet.startposX = message.data.posX;
                    	  bullet.startposY = message.data.posY;
                    	  bullet.startposZ = message.data.posZ;
                      }
                      //if(bullet.time > 0)
                      /*{
                    	  bullet.start_posX[bullet.time] = message.data.posX;
                    	  bullet.start_posY[bullet.time] = message.data.posY;
                    	  bullet.start_posZ[bullet.time] = message.data.posZ;
                      }*/
...
//          System.out.println("bullet "+ bullet);
        ...
        if(message.key == 444){
        	...
        	if (en != null && en instanceof EntityGVCLivingBase) {// 1
        		...
        		if(en.getControllingPassenger() != null && en.getControllingPassenger() == player) {
        ...
        if(message.key == 600){
        	...
        	if (en != null && en instanceof EntityGVCLivingBase) {// 1
        		EntityGVCLivingBase gvc_en = (EntityGVCLivingBase) en;
        		if(en.getControllingPassenger() != null  && en.getControllingPassenger() == player) {
        			gvc_en.setMoveForward_PL(message.Forward);
            		gvc_en.setMoveStrafing_PL(message.Strafing);
        		}
        	}
        }
        ...
        if(message.key == 3301){
        	...
        	if (en != null && en instanceof EntityGVCLivingBase) {// 1
        		EntityGVCLivingBase base = (EntityGVCLivingBase) en;
        		base.getDataManager().set(EntityGVCLivingBase.aitype, Integer.valueOf(message.da));
        	}
        }
        if(message.key == 3302){
        	EntityLivingBase en = (EntityLivingBase) player.world.getEntityByID(message.fre);
        	if (en != null && en instanceof EntityGVCLivingBase) {// 1
        		EntityGVCLivingBase base = (EntityGVCLivingBase) en;
        		base.getDataManager().set(EntityGVCLivingBase.aitype2, Integer.valueOf(message.da));
        	}
        }
        if(message.key == 3303){
        	EntityLivingBase en = (EntityLivingBase) player.world.getEntityByID(message.fre);
        	if (en != null && en instanceof EntityGVCLivingBase) {// 1
        		EntityGVCLivingBase base = (EntityGVCLivingBase) en;
        		base.getDataManager().set(EntityGVCLivingBase.aitype3, Integer.valueOf(message.da));
        	}
        }
