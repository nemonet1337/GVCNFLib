	...
	public static void load(ItemGunBase gun, RegistryEvent.Register<Item> event, String text, String[] type, String scopedomain, String entitydomain) {
		...
		net.minecraftforge.fml.common.ProgressManager.ProgressBar bar= net.minecraftforge.fml.common.ProgressManager.push("Gun Loading", 1, true);
					if (type.length != 0) {// 1
						...
						//missile用
						if (type[0].equals("missile_targeting_time_start")) {
							gun.missile_targeting_time_start = Integer.parseInt(type[1]);
						}
						if (type[0].equals("missile_targeting_time_end")) {
							gun.missile_targeting_time_end = Integer.parseInt(type[1]);
						}
						
						if (type[0].equals("missile_targeting_size")) {
							gun.missile_targeting_size = Float.parseFloat(type[1]);
						}
