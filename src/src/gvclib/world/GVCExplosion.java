...
public class GVCExplosion extends Explosion
{
        ...
        for (int k2 = 0; k2 < list.size(); ++k2)
        {
            ...
            if (!entity.isImmuneToExplosions())
            {
            	if (entity instanceof EntityItem) {
            		...
            		if(entityitem.getItem().getItem() != Item.getItemFromBlock(Blocks.DIRT)
            				 &&entityitem.getItem().getItem() != Item.getItemFromBlock(Blocks.SAND)
            				 &&entityitem.getItem().getItem() != Item.getItemFromBlock(Blocks.SANDSTONE)
            				 &&entityitem.getItem().getItem() != Item.getItemFromBlock(Blocks.COBBLESTONE)
            				 &&entityitem.getItem().getItem() != Item.getItemFromBlock(Blocks.STONEBRICK)
            						 &&entityitem.getItem().getItem() != Item.getItemFromBlock(Blocks.PLANKS)
            				 ){
            			continue;
            		}
            	}
    ...
    public void spawnExp(float sizeid) {
    	if(! this.world.isRemote){
    		if(sizeid >= 2) {
    			for (int i = 0; i < 6; ++i)
    	        {
    	            double d0 = this.x + (this.world.rand.nextDouble() - this.world.rand.nextDouble()) * 3.0D;
    	            double d1 = this.y + (this.world.rand.nextDouble() - this.world.rand.nextDouble()) * 3.0D;
    	            double d2 = this.z + (this.world.rand.nextDouble() - this.world.rand.nextDouble()) * 3.0D;
    	            EntityP_Explosion particle = new EntityP_Explosion( this.world);
        			particle.setLocationAndAngles(d0,  d1,  d2,  0.0F, 0.0F);
        			particle.startposX =  d0;
        			particle.startposY =  d1;
        			particle.startposZ =  d2;
        			particle.timemax = 5 + this.world.rand.nextInt(4);
        			particle.setExpSize(this.size);
        			 this.world.spawnEntity(particle);
    	        }
    		}
    		EntityP_Explosion particle = new EntityP_Explosion( this.world);
			particle.setLocationAndAngles( this.x,  this.y,  this.z,  0.0F, 0.0F);
			particle.startposX =  this.x;
			particle.startposY =  this.y;
			particle.startposZ =  this.z;
			particle.timemax = 8;
			particle.setExpSize(this.size);
			 this.world.spawnEntity(particle);
		}
    ...
    public void doExplosionB(boolean spawnParticles)
    {
        ...
        if (this.size >= 2.0F && this.damagesTerrain)
        {
           ...
           /* {//23/06/17　追加パーティクル消える問題対策
            	if (!this.world.isRemote && this.world instanceof WorldServer)
                {
           		 WorldServer server = (WorldServer) this.world;
           		 server.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, true, this.x, this.y, this.z, 1, 1.0D, 0.0D, 0.0D, 0D, 0);
                }
            }*/
        }
        else
        {
            ...
            /*{//23/06/17　追加パーティクル消える問題対策
                ...
                {
           		 WorldServer server = (WorldServer) this.world;
           		 server.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, true, this.x, this.y, this.z, 1, 1.0D, 0.0D, 0.0D, 0D, 0);
                }
            }*/
        }
    ...
    //                this.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d0, d1, d2, d3, d4, d5);
                    /*{//23/06/17　追加パーティクル消える問題対策
                        ...
                        {
                   		 WorldServer server = (WorldServer) this.world;
                   		 server.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, true, (d0 + this.x) / 2.0D, (d1 + this.y) / 2.0D, (d2 + this.z) / 2.0D, 1, d3, d4, d5, 0D, 0);
                   		server.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, true, d0, d1, d2, 1, d3, d4, d5, 0D, 0);
                        }
                    }*/
                }
                ...
                if (iblockstate.getMaterial() != Material.AIR)
                {
                    if (block.canDropFromExplosion(this))
                    {
                    	if(block == Blocks.SAND 
                    			...
                    			|| block == Blocks.SILVER_GLAZED_TERRACOTTA
